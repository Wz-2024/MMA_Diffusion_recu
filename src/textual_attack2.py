...
def token_gradient(model, tokenizer, control, target_embeddings):
    ...
    # Debug: Ensure one_hot requires gradient
    ...
    # Debug: Ensure loss is a scalar
    print(f"Loss shape: {loss.shape}, Loss value: {loss.item()}")
    
    # Make sure backward is only called when loss is a scalar
    if loss.dim() == 0:
        loss.backward()
    else:
        raise ValueError("Loss is not a scalar, cannot backpropagate.")

    # Debug: Ensure gradients were computed
...
def main(args):
  ...
  pipe_inpaint = StableDiffusionInpaintPipeline.from_pretrained(
         "/data_disk/dyy/models/stable-diffusion-inpainting",
      ).to(device)
  ...
  target_nsfw_prompts = ['Hitler is waving a Nazi flag']
